  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 max-w-5xl mx-auto">
      <div className="text-center mb-16 animate-[fadeIn_0.5s_ease-out]">
        <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4 font-pixel">MISSION FILES</h1>
        <div className="h-2 w-40 bg-slate-900 mx-auto border-2 border-white shadow-[2px_2px_0px_0px_#000]"></div>
        <p className="mt-4 text-slate-500 font-mono">Select a mission to view details...</p>
      </div>
          ...
          <div key={project.id} className="bg-[#c0c0c0] p-1 shadow-retro border-2 border-white outline outline-2 outline-slate-900">
            ...
            <div className="bg-white border-2 border-slate-500 p-6 md:p-8 shadow-[inset_2px_2px_0px_0px_#000000]">
              <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6 border-b-2 border-slate-900 pb-4 border-dashed">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900 font-sans">{project.company}</h2>
                  <p className="text-lg text-blue-700 font-bold font-mono mt-1">ROLE: {project.role}</p>
                </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-slate-700 leading-relaxed font-medium font-mono">
                    {project.description}
                  </p>
                </div>

                <div>
                  <h4 className="text-xs font-bold bg-slate-200 inline-block px-2 py-1 mb-3 border border-slate-900 font-pixel">ACHIEVEMENTS</h4>
                  <ul className="space-y-3">
                    {project.achievements.map((achievement, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-slate-700 bg-[#f0f0f0] p-3 border border-slate-400 shadow-sm">
                        <span className="text-green-600 font-bold font-pixel text-xs mt-1">OK</span>
              ...
              <div className="mt-6 pt-2 border-t border-slate-300 flex justify-between text-xs text-slate-500 font-mono">
                ...
                <span>Files: {project.achievements.length}</span>
              </div>
            </div>
          </div>
        ))}
